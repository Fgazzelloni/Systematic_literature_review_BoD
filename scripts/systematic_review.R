...
df <- read_csv("Included_literature_CDs_reviewed.csv")

head(df)
names(df)

IHME_article<-df%>%filter(Journal=="IHME")
IHME_url<-IHME_article$URL

browseURL(IHME_url)


df%>%filter(Journal=="IHME")

df[str_detect(df,"IHME")]

filter(df,grepl("IHME",Journal))

df[str_detect(df,"healthdata")]
filter(df,grepl("healthdata",URL))
