...
options(scipen = 999)
head(df_cause_and_pop)

my_df<-df_cause_and_pop%>%
  filter(!location=="European Union") %>%
  group_by(location,cause)%>%summarize_if(is.numeric, mean, na.rm = TRUE)
...
p <- ggplot(data=my_df,aes(x=Deaths,y=DALY,size=pop_value)) +
  ...
  theme(plot.background = element_rect(fill = "wheat"),
        ...
        plot.title=element_text(face="bold",size= 18,family="Comic Sans MS",color="thistle4"),
...
####### SAVING ######################################
ragg::agg_png(here::here("day25_dempgraphic", "dempgraphic_day25.png"),
